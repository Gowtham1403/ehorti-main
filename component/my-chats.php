...
<div class="seller-chat">
    <div class="container">
        <div class="chat-wrapper">
            ...
            <div class="chat-body chat-index" id="chat-body">
                <?php 
                  $getCustomer = $con -> query("SELECT DISTINCT customer_email,product_id FROM chat WHERE seller_email = '$_SESSION[userEmail]'");
                    if($getCustomer -> num_rows > 0){
                        while($rowCustomer = $getCustomer -> fetch_assoc()){
                            $getCustomerEmail = $con -> query("SELECT * FROM `chat` WHERE product_id='$rowCustomer[product_id]' and  customer_email = '$rowCustomer[customer_email]' and seller_email = '$_SESSION[userEmail]' ORDER BY id DESC LIMIT 1");
                    if($getCustomerEmail -> num_rows > 0){
                        while($rowCustomerEmail = $getCustomerEmail -> fetch_assoc()){
                            $getCustomerName = $con -> query("select * from `customer-info` where email = '$rowCustomerEmail[customer_email]'");
                            if($getCustomerName -> num_rows > 0){
                                while($rowCustomerName = $getCustomerName -> fetch_assoc()){
                                    echo '<div style="position: relative;">
                                    <a href="customer-chat.php?cid='.$rowCustomerName['id'].'&pid='.$rowCustomerEmail['product_id'].'" class="customer-list"></a>
                                    <div class="row py-2">
                                        <div class="col-2">
...
echo '<script src="../server/send/chat.js"></script>';
